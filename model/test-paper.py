...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
       ...
       # final_pred = np.mean(preds_tta, axis=0)
        c,final_pred = self.model.predict_generator(gen_2, steps=self.validation_steps)
...
'''
base_model = sm.Unet('efficientnetb0',input_shape=(img_height, img_width, chnnel_number),encoder_weights='imagenet')
for layer in base_model.layers:
    layer.trainable = True
    layer.name = layer.name + str("_unet")
concate=Concatenate()([base_model.input,
                       base_model.get_layer(index=-1).output])
prediction1=Dense(class_number,activation='softmax',name='unet-pred')(Dropout(0.2)(Flatten()(base_model.get_layer('top_activation_unet').output)))
effb3 = EfficientNetB3(input_shape=(img_height,img_width,int(concate.shape[-1])),weights=None,include_top=False)(concate)
prediction2=Dense(class_number,activation='softmax',name='main-pred')(Dropout(0.2)(Flatten()(effb3)))
model=Model(inputs=base_model.input,outputs=[prediction1,prediction2])
model.summary()
#model = multi_gpu_model(ppmodel,gpus=4)
model.compile(loss=['categorical_crossentropy','categorical_crossentropy'],loss_weights=[1,1],optimizer=optimizers.SGD(lr=1e-3, momentum=0.9),metrics=['accuracy'])
class ParallelModelCheckpoint(ModelCheckpoint):
    def __init__(self,model,filepath, monitor='val_main-pred_loss', verbose=0,
...
checkpoint = ParallelModelCheckpoint(model,filepath=weight_path+'/smr_{}'.format(label_smooh_rate)+'_{epoch:02d}_valacc_{val_main-pred_acc:.4f}_valloss_{val_main-pred_loss:.4f}.h5',monitor='val_main-pred_loss',verbose=1)
cbks = [early_stopping,val_metrics,checkpoint,CosineAnnealingScheduler(T_max=50, eta_max=1e-3, eta_min=3e-4)]
def gen_smooth_labels(datagen):
    while True:
        iamge,label = datagen.next()
        yield iamge,[smooth_positive_labels(label,label_smooh_rate),smooth_positive_labels(label,label_smooh_rate)]
