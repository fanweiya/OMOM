...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.validation_generator.batch_size=512
       ...
       # final_pred = np.mean(preds_tta, axis=0)
        final_pred = self.model.predict_generator(gen_2, steps=self.validation_steps)
        ...
        for n in error_sample:
            plt.figure(figsize=(50,50))
            plt.subplot(len(error_sample)/5, 5, n + 1)
            # Convert the range from -1 to 1 to the range from 0 to 1
            plt.imshow(load_img(gen_1.filepaths[n],target_size=(224,224)))
            predicted_label = self.labels[y_pred[n]]
            true_label = self.labels[y_true[n]]
            score=final_pred[n][y_pred[n]]
            plt.title('{}\n{}{:.2f}---{}'.format(gen_1.filepaths[n],predicted_label,score
                    ,true_label), color='red')
            plt.axis('off')
        plt.suptitle('Model error predictions (predicted_label,score---true_label)')
        plt.savefig(self.save_path+'/model-'+str(int(epoch)+1)+'predictions-error-result.png')
        ...
        plt.savefig(self.save_path+'/model-epoch-'+str(int(epoch)+1)+'-result.png')
...
input_tensor = Input(shape=(img_height, img_width, chnnel_number))
efnet1=EfficientNetB0(weights='imagenet',input_tensor=input_tensor,include_top=False)
efnet2=EfficientNetB3(weights=None,input_tensor=input_tensor,include_top=False)
for layer in efnet1.layers:
    #layer.trainable = True
    layer.name = layer.name + str("_1")
model_efnet1 = Model(inputs=input_tensor, outputs=efnet1.output)
model_efnet2 = Model(inputs=input_tensor, outputs=efnet2.output)
model_efnet1.compile(loss='categorical_crossentropy', optimizer='adam')
model_efnet2.compile(loss='categorical_crossentropy', optimizer='adam')

    
cnn_dot_out = Lambda(batch_dot)([model_efnet1.output, model_efnet2.output])

sign_sqrt_out = Lambda(sign_sqrt)(cnn_dot_out)
l2_norm_out = Lambda(l2_norm)(sign_sqrt_out)
flatten = Flatten()(l2_norm_out)
dropout = Dropout(0.5)(flatten)
output = Dense(class_number, activation='softmax')(dropout)
...
checkpoint = ParallelModelCheckpoint(model,
                                     filepath=weight_path+'/smr_{}'.format(label_smooh_rate)+'_{epoch:02d}_valacc_{val_acc:.4f}_valloss_{val_loss:.4f}.h5',monitor='val_loss',verbose=1)
cbks = [early_stopping,val_metrics,checkpoint,CosineAnnealingScheduler(T_max=10, eta_max=1e-3, eta_min=3e-4)]
