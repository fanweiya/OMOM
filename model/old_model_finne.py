...
def plot_confusion_matrix(cm, classes,normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig(weight_path+'/model-{}-time-{}.png'.format(str(root.split(os.sep)[-1]),str(img_time)))
    plt.close()
        #plt.show()
class Metrics(Callback):
    def __init__(self, validation_generator, validation_steps,threshod=0.5):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        y_pred = (self.model.predict_generator(gen_2, steps=self.validation_steps)>self.threshod).astype('int')
...
model = load_model('/root/CEST_weights/39/new/new_B7_weights_017_valacc_0.9522_valloss_0.2120.h5')
#model.summary()
#model = multi_gpu_model(ppmodel,gpus=4)
model.compile(loss='categorical_crossentropy',optimizer=optimizers.SGD(lr=0.1, momentum=0.9),metrics=['accuracy',top3_acc,top5_acc])
...
if root.split(os.sep)[-1]=='xc_1205_dataset':
    ...
    checkpoint = ParallelModelCheckpoint(model,filepath=weight_path+'/weights_{}'.format(label_smooh_rate)+'_{epoch:02d}_valacc_{val_acc:.4f}_valloss_{val_loss:.4f}.h5',monitor='val_loss',verbose=1)
cbks = [early_stopping,lr_reduce,checkpoint,val_metrics,CosineAnnealingScheduler(T_max=100, eta_max=1e-2, eta_min=1e-4)]
