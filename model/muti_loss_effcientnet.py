...
lr=3e-4
Num_epochs=50
backbone="EfficientNetB0_mutilabel"
batch_size = 32
chnnel_number = 3
...
def plot_confusion_matrix(cm, classes,epochs,normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig(weight_path+'/model-{}-{}-time-{}.png'.format(str(root.split(os.sep)[-1]),str(epoch),str(img_time)))
    plt.close()
    #plt.show()
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        y_true = np.vstack(next(gen_1)[1][0] for _ in range(self.validation_steps)).astype('int')
        prd_model = Model(inputs=self.model.input[0],outputs=self.model.get_layer('dense_1').output)
        y_pred = (pre_model.predict_generator(gen_2, steps=self.validation_steps)>self.threshold).astype('int')
        ...
        print(report)
        plot_confusion_matrix(cm,classes=self.validation_generator.class_indices.keys(),epochs=self.epoch)
        return
...
predicton_masterlabel_output=Dense(2,activation='sigmoid',name="sigmoid")(Flatten()(efnet.get_layer('block6b_add').output))
model = Model(efnet.inputs,[prediction_output,predicton_masterlabel_output])
#model.summary()
#model = multi_gpu_model(ppmodel,gpus=4)
#lr_normalizer(params['lr'],params['optimizer']))
model.compile(loss=['categorical_crossentropy','binary_crossentropy'],loss_weights=[1,1],optimizer=optimizers.Adam(lr=lr),metrics=['accuracy'])
...
def gen_data_labels(datagen):
    while True:
        ...
        yield [iamge,[smooth_positive_labels(labels,label_smooh_rate),master_labels]]
