...
label_smooh_rate=0
Num_epochs=epoch_size=50
backbone="old_model_3label"
batch_size = 32
chnnel_number = 3
...
class Metrics(Callback):
    def __init__(self, validation_generator, validation_steps,treshod=0.5):
        self.validation_generator = validation_generator
        self.validation_steps = validation_steps
        self.treshod=treshod
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        pred,b,c=self.model.predict_generator(gen_2, steps=self.validation_steps)
        y_pred = (pred>self.treshod).astype('int')
...
schedule = SGDRScheduler(min_lr=1e-5,
                                     ...
                                     cycle_length=5,
                                     mult_factor=1.5)
efnet=load_model('/root/CEST_weights/39/new/new_B7_weights_017_valacc_0.9522_valloss_0.2120.h5')
...
model.compile(loss=['categorical_crossentropy','binary_crossentropy','categorical_crossentropy'],loss_weights=[1,1,1],optimizer=optimizers.SGD(lr=0.1, momentum=0.9),metrics=['accuracy',top3_acc,top5_acc])
...
cbks = [early_stopping,lr_reduce,checkpoint,schedule]
