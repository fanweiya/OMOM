...
train_datagen = ImageDataGenerator(
    ...
    cval=0,
    rescale=1. / 255,
    width_shift_range=np.random.choice(np.linspace(0, 0.3,5)),
    height_shift_range=np.random.choice(np.linspace(0, 0.3,5)),
...
#model.summary()
'''
model=load_model('/share_data/PRDATA/fanweiya/CEST_weights/TrainB4-newdata/model_2020-04-13-13:22:35/smr_0_09_valacc_0.8607_valloss_0.3107.h5')
'''
...
checkpoint = ParallelModelCheckpoint(model,filepath=weight_path+'/fcocal_smr_{}'.format(label_smooh_rate)+'_{epoch:02d}_valacc_{val_acc:.4f}_valloss_{val_loss:.4f}.h5',monitor='val_loss',save_best_only=True,verbose=1)
tensorboard_log=TensorBoard(log_dir=weight_path+'/model_log')
cbks1 = [early_stopping,checkpoint,lr_reduce,val_metrics]
cbks = [early_stopping,checkpoint,lr_reduce,val_metrics,lr_scheduler,tensorboard_log]
...
'''
optimizers = RAdam(learning_rate=lr_schedule(0)*gpu_num)
#optimizers = optimizers.SGD(lr=lr_schedule(0)*gpu_num,momentum = 0.9, nesterov = True)
for layer in Model.layers:  
    layer.trainable = True
Model.compile(loss=categorical_focal_loss(gamma=2.0, alpha=0.25),optimizer=optimizers,metrics=['acc'])
'''
