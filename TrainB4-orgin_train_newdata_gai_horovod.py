...
#model.summary()
'''
base_model=load_model('/share_data/CEST_weights/TrainB4-newdata/model_2020-03-12-20:25:27/jc_smr_0_03_valacc_0.4946_valloss_2.1482.h5')
...
cbks = [early_stopping,checkpoint,lr_reduce,val_metrics,cosin_lr,lr_scheduler]
callbacks = [
...
'''
opt=optimizers.SGD(lr=lr_schedule(0)* hvd.size(),momentum = 0.9, nesterov = True)
...
history=Model.fit_generator(gen_smooth_labels(train_generator),epochs=Num_epochs,
                                   steps_per_epoch=totalTrain // hvd.size(),
